...
export default function Class() {
  ...
  const [classi, {  refetch:getActionFetch }] = createResource(pagech,getAction);
  const [row,{refetch:getActionTotalFetch} ] = createResource(pagech,getActionTotal);
  ...
  async function getAction() {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/action/action`+pagesearch(),{
        method: 'GET',
        headers: {
    ...
    async function getActionTotal() {
      
      const response = await fetch(
        `${import.meta.env.VITE_API_ENDPOINT}/action/actiontotal`+pagesearch(),{
          method: 'GET',
          headers: {
            'Content-type': 'application/json; charset=UTF-8',
            "Authorization": `Bearer ${sessionStorage.getItem('sessionId')}`
  ...
  socket.onmessage = (event) => {
    ...
    if(res.actionadd != undefined || res.actionsub != undefined   ){
      getActionFetch()
      getActionTotalFetch()
    }
   
    if(res.actionid != undefined){
      let myModal = document.getElementById("tableviewid")
       console.log(myModal)
   
      if(idquery.id==res.actionid){
        tableViewIdquery(res.actionid)
...
socket.addEventListener("error", event => {console.log(event)});
  ...
  const tableViewIdquery =(id:number)=>{
    fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/getIdAction/${id}`
      ,{
        headers:{
        "Authorization": `Bearer ${sessionStorage.getItem('sessionId')}`
      }}
      ...
      <div class="modal fade" id="tableviewid" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <form onSubmit={update} id="eaction" class="need-validation" noValidate>
            <Show when={alerton()}>
            ...
            <button type="submit" class="btn btn-primary" form="eaction" >Save</button>
  ...
  const deleteData =(id:number)=>{
    fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/deleteAction`,{
        method: 'POST',
        body: JSON.stringify({
          id:id
        }),
       ...
       getActionFetch()
       getActionTotalFetch()
       socket.send(JSON.stringify({actionsub: "update"})); 
      })
  }
  const update =(event:any)=>{
    ...
    fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/updateAction`,{
        method: 'POST',
        body: JSON.stringify({
          idquery:idquery
        }),
        ...
        if(res.status=='ER_DUP_ENTRY'){
          ...
          getActionFetch()
          getActionTotalFetch()
          socket.send(JSON.stringify({actionid: idquery.id})); 
        }
  ...
  const create =(event:any)=>{
    ...
    fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/createAction`,{
        method: 'POST',
        body: JSON.stringify({
          idquery:idquery
        }),
        ...
        if(res.status=='ER_DUP_ENTRY'){
          ...
          socket.send(JSON.stringify({actionadd: "update"})); 
        }
            ...
            <div class="modal fade" id="create" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                  <form onSubmit={create} id="auser" class="need-validation" noValidate>
                  <Show when={alerton()}>
                      <div class="alert alert-warning" role="alert">
                        type is already exist
                      </div>
                ...
                <div class="modal-content">
                  ...
                  <div class="mb-3 row">
                      ...
                      <div class="col-sm-10">
                        ...
                        <input type="text" id="ettype" class="form-control" value={idquery.ttype}  onChange={(e)=>{setidquery(produce((vald)=>{vald.ttype= e.target.value}))}}  required/>
                      </div>
                    </div>
                    <div class="mb-3 row">
                      <label for="edesc" class="col-sm-2 col-form-label">Description</label>
                      <div class="col-sm-10">
                        <input type="text" id="edesc"  class="form-control"  value={idquery.description}  onChange={(e)=>{setidquery(produce((vald)=>{vald.description= e.target.value}))}}  required/>
                      </div>
                    </div>
                </form>
                  </div>
                  <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" onClick={()=>{ location.href="action?page=1"}}>Reset</button>
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" onClick={() => { location.href="action?page=1&desc="+idquery.description+'&ttype='+idquery.ttype}} >Search</button>
                  </div>
                </div>
              </div>
            </div>
  ...
  const res = await fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/printAction${pagesearch()}`, {
        method: 'GET',
        headers: {
          'Content-type': 'application/json; charset=UTF-8',
          "Authorization": `Bearer ${sessionStorage.getItem('sessionId')}`
  ...
  const toCVS = async () =>{
    let res = await exportData()
      ...
      let newarr = []
      newarr.push("Action Type Master",'\n','Report of ',formattedDate,'\n')
  ...
  return (
   ...
   {row.loading? <p>Loading...</p>:(
      <RightDesign csv={toCVS} pdf={toPDF} tableViewId={tableViewId} pageroute={"Action Type Master"} print={toPrint} tableview={tableview(classi())} tablerow={tableviewrows(row(),pagesearch())} tablecreate={tablecreate()} tablesearch={tablesearch}></RightDesign>
    )} 
    </Body>
   </CredentialUser>
