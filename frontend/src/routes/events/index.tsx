...
export default function Class() {
  ...
  const [classi, {  refetch:getEventFetch }] = createResource(pagech,getEvent);
  const [row,{refetch:getEventTotalFetch} ] = createResource(pagech,getEventTotal);
  const [showt, setshowt] = createSignal(false);
  async function getEvent() {
      const response = await fetch(
        `${import.meta.env.VITE_API_ENDPOINT}/event/event`+pagesearch(),{
          method: 'GET',
          headers: {
  ...
  async function getEventTotal() {
 
      const response = await fetch(
        `${import.meta.env.VITE_API_ENDPOINT}/event/eventtotal`+pagesearch(),{
          method: 'GET',
          headers: {
            'Content-type': 'application/json; charset=UTF-8',
            "Authorization": `Bearer ${sessionStorage.getItem('sessionId')}`
  ...
  socket.onmessage = (event) => {
    ...
    if(res.eventadd != undefined || res.eventsub != undefined   ){
      getEventFetch()
      getEventTotalFetch()
    }
    ...
    if(res.eventid != undefined){
      ...
      if(idquery.id==res.eventid){
        tableViewIdquery(res.eventid)
...
socket.addEventListener("error", event => {console.log(event)});
  ...
  const tableViewIdquery =(id:number)=>{
    fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/getIdEvent/${id}`,{
        headers:{
          "Authorization": `Bearer ${sessionStorage.getItem('sessionId')}`
        }
      }).then(async (data)=>{
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h1 class="modal-title fs-5" id="staticBackdropLabel">Edit events</h1>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" ></button>
            </div>
            <div class="modal-body">
            <form id="eevents" onSubmit={update} class="need-validation" noValidate>
                  <Show when={alerton()}>
            ...
            <button type="submit" class="btn btn-primary" form="eevents" >Save</button>
  ...
  const deleteData =(id:number)=>{
    fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/deleteEvent`,{
        method: 'POST',
        body: JSON.stringify({
          id:id
        }),
       ...
       socket.send(JSON.stringify({eventsub: "update"})); 
      })
  }
  const update =(event:any)=>{
    ...
    fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/updateEvent`,{
        method: 'POST',
        body: JSON.stringify({
          idquery:idquery
        }),
        ...
        if(res.status=='ER_DUP_ENTRY'){
          ...
          socket.send(JSON.stringify({eventid: idquery.id})); 
        }
  ...
  const create =(event:any)=>{
    ...
    fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/createEvent`,{
        method: 'POST',
        body: JSON.stringify({
          idquery:idquery
        }),
        ...
        if(res.status=='ER_DUP_ENTRY'){
          ...
          socket.send(JSON.stringify({eventadd: "update"})); 
        }
            ...
            <div class="modal fade" id="create" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                  <form onSubmit={create} id="adduser" class="need-validation" noValidate>
                  <Show when={alerton()}>
                      <div class="alert alert-warning" role="alert">
                        type is already exist
                      </div>
            ...
            <div class="modal fade" id="search" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" onClick={()=>{ location.href="events?page=1"}}>Reset</button>
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" onClick={() => { location.href="events?page=1&desc="+idquery.description+'&ttype='+idquery.ttype}} >Search</button>
                  </div>
                </div>
              </div>
            </div>
  ...
  const res = await fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/event/printEvent${pagesearch()}`, {
        method: 'GET',
        headers: {
          'Content-type': 'application/json; charset=UTF-8',
          "Authorization": `Bearer ${sessionStorage.getItem('sessionId')}`
  ...
  return (
   ...
   {row.loading? <p>Loading...</p>:(
      <RightDesign csv={toCVS} pdf={toPDF} tableViewId={tableViewId} pageroute={"Event Type Master"} print={toPrint} tableview={tableview(classi())} tablerow={tableviewrows(row(),pagesearch())} tablecreate={tablecreate()} tablesearch={tablesearch}></RightDesign>
    )} 
    </Body>
   </CredentialUser>
