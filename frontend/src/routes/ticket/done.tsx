...
export default function Ticket() {
  ...
  async function getTicketTotal() {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/ticket/tickettotaldone`+URlparms(),{
        method: 'GET',
        headers: {
  ...
  async function getTicket() {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_ENDPOINT}/ticket/ticketdone`+URlparms(),{
          method: 'GET',
...
socket.addEventListener("error", event => {console.log(event)});
 ...
 // @ts-ignore
        let adate = formatter.format(new Date(res.datecomplete )).replaceAll("/", "-")
        const [ month3, day3,year3] = adate.split(/\D/);
        ...
        res.datecomplete = `${year3}-${month3.length==1?`0${month3}`:month3}-${day3.length==1?`0${day3}`:day3}`;
          ...
          <div class="modal-content" style={{width:"40rem"}}>
            ...
            <div class="mb-3 row">
                      ...
                      <div class="mb-3 row">
                        ...
                        <div class="mb-3 row">
                        <label  class="col-sm-3 col-form-label">Date Actual End</label>
                        <div class="col-sm-9">
                            
                          <input type="date"  class="form-control" value={idquery.datecomplete} readOnly/>
                        </div>
                      </div>
                    <div class="mb-3 row">
            ...
            <div class="modal fade" id="search" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true" >
              <div class="modal-dialog" >
                <div class="modal-content"  style={{width:"40rem"}}>
                  ...
                  <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" onClick={()=>{ location.href="done?page=1"}}>Reset</button>
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onClick={closebtn}>Close</button>
                    <button type="button" class="btn btn-primary" onClick={() => { location.href="done?page=1&proid="+idquery.proid+'&ticketno='+idquery.ticketno
  ...
  const res = await fetch(
      `${import.meta.env.VITE_API_ENDPOINT}/ticket/printTicketdone${URlparms()}`, {
        method: 'GET',
        headers: {
          'Content-type': 'application/json; charset=UTF-8',
          "Authorization": `Bearer ${sessionStorage.getItem('sessionId')}`
  ...
  return (
   ...
   {row.loading? <p>Loading...</p>:(
      <RightDesign csv={toCVS} pdf={toPDF} tableViewId={tableViewId} pageroute={"Ticket"} print={toPrint} tableview={tableview(classi())}  tablerow={tableviewrows(row(),URlparms())} tablecreate={<div></div>} tablesearch={tablesearch} filtersearch={filterPro}></RightDesign>
    )} 
    </Body>
   </CredentialUser>
   </>
